...
static NSInteger kFacebookLogoutAlertTag = 1002;
static NSInteger kTwitterLogoutAlertTag = 1003;
...
- (void)showConfirmLogoutDialogForService:(NSString*)service handler:(void(^)())handler {
    ...
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Are You Sure?" message:message delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Log Out", nil];
    if ([service isEqualToString:@"Facebook"]) {
        alertView.tag = kFacebookLogoutAlertTag;
    }
    else {
        alertView.tag = kTwitterLogoutAlertTag;
    }
    [alertView show];
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    else if (alertView.tag == kFacebookLogoutAlertTag) {
        WEAK(self) weakSelf = self;
        if (buttonIndex == alertView.cancelButtonIndex) {
            [weakSelf updateInterfaceToReflectSessionStatuses];
        }
    ...
    else if (alertView.tag == kTwitterLogoutAlertTag) {
        WEAK(self) weakSelf = self;
        if (buttonIndex == alertView.cancelButtonIndex) {
            [weakSelf updateInterfaceToReflectSessionStatuses];
        }
